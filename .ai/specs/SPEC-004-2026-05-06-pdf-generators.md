...
<any module>/:id (detail view)
  ...
  <ModulePdfTabWidget>
    ...
    └── TemplatesList(templateIds, data)
          ├── GET /api/pdf-generators/templates → TemplateMeta[] (filtered by templateIds)
...
├── data/
│   └── quote-detail/
│       ├── index.ts             # re-exports document-data + types
...
```ts
// data/quote-detail/document-data.ts
export function toDocumentData(record: QuoteWidgetRecord): Record<string, unknown>
```

Maps `context.record` from the injection framework to the template's expected shape. Lives in `data/<context-name>/` — not inside the widget folder, making it reusable.
...
1. **Template list** — card grid fetched from `GET /api/pdf-generators/templates`, filtered by `templateIds` prop.
...
1. `components/TemplatesList.tsx` — fetches templates via `GET /api/pdf-generators/templates`, filters by `templateIds`, renders card list
...
7. `data/quote-detail/` — `types.ts`, `document-data.ts`, `index.ts` (data mapper for quote context)
8. `widgets/injection/quote_pdf_tab/` — first reference widget: `widget.ts`, `widget.client.tsx`
